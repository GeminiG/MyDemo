...
import com.example.demo.redisdemo.util.RedisUtil;
...
public class RedisTestDemo {
    ...
    private static final int RECORD_SIZE = 100000;
    ...
    public void addValueToRedis() {
        ...
        for (int i = 0; i < RECORD_SIZE; i++) {
            redisUtil.set(String.valueOf(i), value);
        }
    ...
    public void getValueFromRedis() {
        ...
        long cost = System.currentTimeMillis() - startTime;
        System.out.println("获取数据共用时：" + cost);
    }
    ...
    public void delete() {
        long startTime = System.currentTimeMillis();
        for (int i = 0; i < RECORD_SIZE; i++) {
            redisUtil.delete(REDIS_KEY_PREFIX + i);
        }
        long cost = System.currentTimeMillis() - startTime;
        System.out.println("删除所有的记录共花费: " + cost);
    }
    ...
    public void pDelete() {
        ...
        List<String> result = redisTemplate.executePipelined(new RedisCallback<Object>() {
            ...
            public String doInRedis(RedisConnection redisConnection) {
                for (int i = 0; i < RECORD_SIZE; i++) {
                    redisConnection.del((i+"").getBytes());
                }
                return null;
            }
        });
    ...
    public void zSetAdd() {
        ...
        for (int i=0; i < RECORD_SIZE; i++) {
            ...
            redisUtil.zSetAdd("TEST_ZSET", i + value , score);
        }
        long cost = System.currentTimeMillis() - startTime;
        System.out.println("zset add 花费的时间为： " + cost);
    }
    ...
    public void pZetAdd() {
        ...
        List<String> result = redisTemplate.executePipelined(new RedisCallback<Object>() {
            ...
            public String doInRedis(RedisConnection redisConnection) {
                ...
                for (int i = 0; i < RECORD_SIZE; i++) {
                    double score = rand.nextDouble();
                    redisConnection.zSetCommands().zAdd("TEST_ZSET".getBytes(), score, (i + value).getBytes());
                }
                return null;
            }
        });
        long cost = System.currentTimeMillis() - startTime;
        System.out.println("使用管道插入zset共花费: " + cost);
    }
    ...
    public void zSetGet() {
        ...
        for (int i = 0; i < RECORD_SIZE; i++) {
            ...
            if (score1 < score2) {
                redisUtil.zSetGet("TEST_ZSET", score1, score2);
            }
            else {
                redisUtil.zSetGet("TEST_ZSET", score2, score1);
            }
        }
        long cost = System.currentTimeMillis() - startTime;
        System.out.println("zset get 花费的时间为： " + cost);
    }
    ...
    public void pZetGet() {
        ...
        List<String> result = redisTemplate.executePipelined(new RedisCallback<Object>() {
            ...
            public String doInRedis(RedisConnection redisConnection) {
                ...
                for (int i = 0; i < 100; i++) {
                    ...
                    double score2 = random.nextDouble();
                    if (score1 < score2) {
                        redisConnection.zSetCommands().zRangeByScore("TEST_ZSET".getBytes(), score1, score2);
                    }
                    else {
                        redisConnection.zSetCommands().zRangeByScore("TEST_ZSET".getBytes(), score2, score1);
                    }
                }
                return null;
            }
        ...
        long cost = System.currentTimeMillis() - startTime;
        System.out.println("使用管道获取记录共花费: " + cost);
    }
}
